...
case $option in
	-s|--start) CUDA_VISIBLE_DEVICES=0,1 torchrun train.py config/train_bash_notes.py --compile=False --eval_iters=20 --log_interval=10 \
	--batch_size=12 --n_layer=6 --n_head=6 --n_embd=840 --max_iters=5000 --lr_decay_iters=5000 --dropout=0.1
	;;
	-k|--kill) for i in {0..9}; do sudo kill -9 $(pgrep torchrun); printf "iter: $i\n"; done;;
esac
